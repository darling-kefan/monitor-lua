...
function initializeMenu(navigatorId) {
	...
	for(var i = 0; i < menus.length; i++) {
		...
		for(var n = 0; n < controllers.length; n++) {
			...
			controllerHTML += '<li><a href="'+ controller.target +'" target="mainContent"><span class="glyphicon glyphicon-align-justify btn-xs"></span><span class="text">'+ controller.name +'</span></a></li>';
		}
		controllerHTML = '<ul class="collapse" id="Menu'+ menu.privilegeId +'">'+ controllerHTML +'</ul>';
		
		var menuHTML = '<li data-navigator="'+ menu.parentId +'"><a href="#Menu'+ menu.privilegeId +'" data-parent="#MenuBox" data-toggle="collapse"><span class="'+ menu.icon +' btn-xs"></span><span class="text">'+ menu.name +'</span></a>'+ controllerHTML +'</li>';
